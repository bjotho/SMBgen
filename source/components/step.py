...
def create_step(step_group, item, level):
    if c.COLOR in item:
        color = item[c.COLOR]
    else:
    ...
    if type == c.TYPE_COIN:
        step_group.add(Step(x, y, type, color, level.coin_group))
    elif (type == c.TYPE_STAR or
          type == c.TYPE_FIREFLOWER or
          type == c.TYPE_LIFEMUSHROOM):
        step_group.add(Step(x, y, type,
                              color, level.powerup_group))
    else:
        if c.STEP_NUM in item:
            create_step_list(step_group, item[c.STEP_NUM], x, y, type,
                              color, item['direction'])
        else:
            step_group.add(Step(x, y, type, color))


def create_step_list(step_group, num, x, y, type, color, direction):
    ''' direction:horizontal, create step from left to right, direction:vertical, create step from up to bottom '''
    size = 43  # 16 * c.STEP_SIZE_MULTIPLIER is 43
    tmp_x, tmp_y = x, y
    for i in range(num):
        if direction == c.VERTICAL:
            ...
            tmp_x = x + i * size
        step_group.add(Step(tmp_x, tmp_y, type, color))

class Step(stuff.Stuff):
    def __init__(self, x, y, type, color=c.ORANGE, group=None, name=c.MAP_STEP):
        # orange_rect = [(16, 0, 16, 16), (432, 0, 16, 16)]
        # blue_rect = [(208, 32, 16, 16), (48, 32, 16, 16)]
        orange_rect = [(0, 16, 16, 16)]
        blue_rect = [(0, 48, 16, 16)]
        if color == c.COLOR_TYPE_ORANGE:
            frame_rect = orange_rect
        else:
            frame_rect = blue_rect
        stuff.Stuff.__init__(self, x, y, setup.GFX['tile_set'],
                             frame_rect, c.STEP_SIZE_MULTIPLIER)
    ...
    def change_to_piece(self, group):
        ...
        for arg in arg_list:
            group.add(StepPiece(*arg))
        self.kill()


class StepPiece(stuff.Stuff):
    def __init__(self, x, y, x_vel, y_vel):
        stuff.Stuff.__init__(self, x, y, setup.GFX['tile_set'],
                             [(68, 20, 8, 8)], c.STEP_SIZE_MULTIPLIER)
        self.x_vel = x_vel
        self.y_vel = y_vel
        self.gravity = .8
